...
The simplest ordinary differential equation (in time) for $\Wtil$ which has the above properties\footnote{FIXME: In the \texttt{-hydrology routing} model a different evolution is needed because the input water $m$ must go directly into till storage, as there is no ``$W$'' variable.  \cite{BBssasliding} includes a specific parameterization already, namely
$$\frac{\partial \Wtil}{\partial t} = \frac{m}{\rho_w} - C$$
where $C>0$ is a decay factor that means that the base will drain without ongoing (small) input.  Actually the right-hand-side is only used to compute $\partial \Wtil/\partial t$ if $\Wtil < \Wtilmax$ or the right-hand-side is negative.  If $\Wtil = \Wtilmax$ and the right-hand-side is positive then we set $\partial \Wtil/\partial t=0$.} is this equation with a constant rate parameter $\mu\ge 0$ and a transfer ratio $\tau\ge 0$:
\begin{equation}
