...
The problem considered in sections \ref{sec:elements} and \ref{sec:capacity} includes a mass conservation equation for the total water amount, a portion of which is transportable, and also it has an equation for the evolution of the water stored locally in till pore spaces.  In this appendix we treat this kind of problem abstractly.  We point out some properties of such a mathematical structure, and we address numerical schemes which preserve important inequalities for the portions of the conserved quantity.

An abstract view of this problem is the following pair of equations for abstract quantities $u(t,x,y),v(t,x,y)$ satisfying bounds $u\ge 0$ and $0 \le v \le v_{\text{max}}$:
\begin{align}
