...
\newpage
\section{Distributed versus routing} \label{app:pismdoc}
...
\subsection*{Model ``\texttt{distributed}''}  This PISM hydrology model is chosen by runtime option \texttt{-hydrology distributed}.  It is governed by the full set of equations \eqref{eq:bluebox} in the main text, that is:
\begin{align}
...
\subsection*{Model ``\texttt{routing}''}  This model is chosen by runtime option \texttt{-hydrology routing}.  It is governed by a subset of the above equations, but with the equation for evolution of pressure $P$ is removed, and with the replacement $P\mapsto P_o$ in defining $K$, $\bV$, and $\psi$.  Thus the equations simplify to:
\begin{align}
\ddt{W} + \ddt{\Wtil} &= - \Div\left(\bV\, W\right) + \Div \left(D \grad W\right) + \frac{m}{\rho_w}, \notag \\
\frac{\partial \Wtil}{\partial t} &= \mu \left(\min\left\{\omega W,\Wtilmax\right\} - \Wtil\right), \notag \\
\tau_c &= c_0 + (\tan \varphi)\, \min\left\{P_o,\, \delta P_o \, 10^{(e_0/C_c) \left(1 - (\Wtil/\Wtilmax)\right)}\right\}, \notag
...
\begin{align*}
K    &= k W^{\alpha-1} \left|\grad(P_o+\rho_w g b)\right|^{\beta-2}, \\
\bV  &= - K \grad\left(P_o + \rho_w g b\right), \\
D    &= \rho_w g K W, \\
\psi &= P_o + \rho_w g (b + W), \\
P_o  &= \rho_i g H.
\end{align*}
...
\subsection*{Model ``\texttt{null}''}  This non-conserving model is chosen by runtime option \texttt{-hydrology null}.  It has only the state variable $\Wtil$.  The  $W \leftrightarrow \Wtil$ transfer equation is replaced by a self-contained evolution equation for $\Wtil$, namely equation \eqref{eq:upbbox}.  There is an additional constant ``drainage'' rate which keeps $\Wtil$ from staying constantly at its maximum value.  Only the determination of basal yield stress is unchanged.  We get
\begin{align}
\ddt{\Wtil} &= \frac{m}{\rho_w} - C \notag \\
\tau_c &= c_0 + (\tan \varphi)\, \min\left\{P_o,\, \delta P_o \, 10^{(e_0/C_c) \left(1 - (\Wtil/\Wtilmax)\right)}\right\}, \notag
\end{align}
along with bounds $0\le \Wtil \le \Wtilmax$, and the same definition:
\begin{align*}
P_o  &= \rho_i g H.
\end{align*}

\subsection*{Correspondence with PISM configuration variables}  FIXME:  I will put a table here
