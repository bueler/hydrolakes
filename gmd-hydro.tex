...
\title{A distributed numerical model of subglacial hydrology \\ in the Parallel Ice Sheet Model}
...
\runningtitle{A model of subglacial hydrology in PISM}
...
\begin{table*}[ht]
\caption{Functions used in subglacial hydrology model \eqref{eq:bluebox}, including symbol, units, and meaning.}
\begin{tabular}{l|l}
\hline
...
\end{equation}
This condition says that the water amount is above a critical level that depends on the sliding and the overburden pressure.  If the pressure effect sliding is large ($s_b \gg P_o$) then $W_c\approx W_r$.  If \eqref{eq:steadyboundfirst} or \eqref{eq:steadyboundsecond} holds then
\begin{equation}
P(W) = P_o - s_b \left(\frac{(W_r - W)_+}{W}\right)^{1/3} \quad \text{in steady state}.  \label{eq:PofWsteady}
\end{equation}
...
\frac{\partial P}{\partial W} =
    \begin{cases}
      ...
      \frac{s_b W_r}{3 W^{4/3} (W_r - W)^{2/3}}, & W_c < W < W_r, \\
      \text{undefined}, & W = W_r, \\
      0, & W > W_r.
...
\begin{align}
\bV &= - k \grad P - \rho_w g k \grad b \label{eq:Vsteadyexpand} \\
    &= - k \bigg[\grad \left(P_o + \rho_w g b\right) \notag \\
    & \qquad \qquad - \left(\frac{W_r - W}{W}\right)^{1/3} \grad s_b \notag \\
    & \qquad \qquad + \frac{s_b W_r}{3 W^{4/3} (W_r - W)^{2/3}} \grad W\bigg]. \notag
\end{align}
Formula \eqref{eq:Vsteadyexpand} helps us understand the steady state meaning of the advective flux ``$\bV W$'' in $\bq=\bV W - D \grad W$.  The direction of water velocity $\bV$ is determined by a combination of a geometric direction ($-\grad \left(P_o + \rho_w g b\right)$), a direction derived from spatial variations in the sliding speed ($\grad s_b$), and a diffusive direction ($-\grad W$).  Therefore a fraction of $\bV W$ is diffusive in steady state, in addition to the \emph{a priori} diffusive flux $- D \grad W$.  To our knowledge, the identification of the diffusiveness of this steady velocity is new to this paper.

Let $\psi_o = P_o + \rho_w g b$.  In steady state we can write the flux as a linear combination of gradients,
\begin{equation}
\bq = \bV W - D \grad W = - A_1 \grad \psi_o + A_2 \grad s_b - A_3 \grad W,  \label{eq:qabstract}
\end{equation}
with coefficients
\begin{align}
&A_1 = k W, \label{eq:qcoefficient} \\
&A_2 = k \left(W_r - W\right)^{1/3} W^{2/3}, \notag \\
...
\subsection{Spatial discretization}  To set notation, suppose the rectangular computational domain has $M_x \times M_y$ gridpoints $(x_i,y_j)$ with uniform spacing $\Delta x,\Delta y$.  Let $\Wlij \approx W(t_l,x_i,y_j)$, $(\Wtil)_{ij}^l \approx \Wtil(t_l,x_i,y_j)$, and $\Plij \approx P(t_l,x_i,y_j)$ denote the numerical approximations.
...
In this section we document the PISM runtime options for hydrology options.  There are three runtime choices, namely \texttt{distributed}, \texttt{routing}, and \texttt{null}, which we list in the order of decreasing complexity.  After that we give a correspondence (concordance) between the symbols in this paper and the relevant PISM configuration parameters.  This is not complete documentation of the user's view of the model; compare the PISM User's Manual \citep{pism-user-manual}.

\subsection{\texttt{distributed}}  This PISM hydrology model is chosen by runtime option \texttt{-hydrology distributed}.  It is governed by the full set of equations \eqref{eq:bluebox} in the main text, that is:
\begin{align}
...
\subsection{\texttt{routing}}  This model is chosen by runtime option \texttt{-hydrology routing}.  It is governed by a subset of the above equations, but with the equation for evolution of pressure $P$ is removed, and with the replacement $P\mapsto P_o$ in defining $K$, $\bV$, and $\psi$.  Thus the equations simplify to:
\begin{align}
\ddt{W} + \ddt{\Wtil} &= - \Div\left(\bV\, W\right) + \Div \left(D \grad W\right) + \frac{m}{\rho_w}, \notag \\
\frac{\partial \Wtil}{\partial t} &= \mu \left(\min\left\{\omega W,\Wtilmax\right\} - \Wtil\right), \notag \\
\tau_c &= c_0 + (\tan \varphi)\, \min\left\{P_o,\, \delta P_o \, 10^{(e_0/C_c) \left(1 - (\Wtil/\Wtilmax)\right)}\right\}, \notag
...
\subsection{PISM configuration variables}  All of the constants in the above model are configurable parameters in PISM versions \texttt{stable0.6} and later.  The correspondence between configuration parameters names and the symbols in this paper is listed in Table \ref{tab:correspondence} below.
...
Englacial water is assumed to fill crevasses and moulins up to a level $z_w(t)$ above the bedrock, in a system which has macroporosity $\phi$ (dimensionless).  Thus the englacial storage is $S_{en}=L W \phi z_w$.  In summary, the \cite{Bartholomausetal2011} model uses this essentially-kinematical equation:
\begin{equation}
S = S_{en} + S_{sub} = L W \phi z_w + \frac{f L W}{h} A_c.  \label{eq:barth:kinematics}
\end{equation}
