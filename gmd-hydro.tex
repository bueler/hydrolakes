...
For convenience only we denote the ice geometry, bed geometry, and sliding speed (i.e.~$H_{i,j}$, $b_{i,j}$, $(P_o)_{i,j}$, and $|\bv_b|_{i,j}$) as though they are all time-independent.  The geometry may be quite general, with ice-free land, floating ice shelf, or ice-free ocean allowed at any location $(x_i,y_j)$.  The geometry input data determines boolean ``masks'' on the grid, based on zero as the sea level elevation:
\begin{align*}
