...
We used the fields $H$, $m$, $|\bv_b|$ as steady data in 5 model year runs of our mass-conserving hydrology \texttt{routing} and \texttt{distributed} models.  Because these fields were fixed, only one-way (ice dynamics model $\to$ subglacial hydrology model) coupling is tested here.

These runs had 1.05 million grid points at which variables $W$, $\Wtil$, and $P$ were recomputed at each time-step.  In both \texttt{routing} and \texttt{distributed} models the hydrological system was seen to become quite steady after the first two model years.  The time-steps for the hydrology model had adaptively-determined (section \ref{sec:num}) steady durations of 4 model hours for the \texttt{routing} model based on maximum subglacial water speeds of 0.05 $\text{m}\,\text{s}^{-1}$.

FIXME: text describing the Greenland 2km runs: routing, distributed
