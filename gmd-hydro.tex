...
FIXME: text describing the Greenland 2km runs: spinup, routing, distributed
...
\caption{To evaluate the result of the spin-up ice dynamical run we compare modelled ice speed at the ice surface (left; $\mathrm{m}\,\mathrm{a}^{-1}$) to satellite observations (right; $\mathrm{m}\,\mathrm{a}^{-1}$).}
...
\caption{The inputs to the hydrology model are the modeled basal melt rate $m/\rho_w$ (left; $\mathrm{m}\,\mathrm{a}^{-1}$) and sliding speed (right; $\mathrm{m}\,\mathrm{a}^{-1}$) from the final state of the spin-up ice dynamical run.}
