...
\end{enumerate}
In Appendix A we detail points (\emph{i}), (\emph{ii}), and (\emph{iii}).  Observation (\emph{iv}) is addressed in section \ref{sec:exactsolution}, next.
...
\item a new analysis of steady state explaining the degree to which a functional relationship $P(W)$ is justified (subsection \ref{subsec:steady}),
\item an exact solution of the coupled mass and pressure equations of the distributed model in the steady angularly-symmetric case (subsection \ref{subsec:steady}), plus verification using this solution (section \ref{sec:results}), and
...
\end{equation}
One may regard $s_b$ as a scale for the pressure drop from cavitation.  Then \eqref{eq:steadyboundfirst} is equivalent to
\begin{equation}
...
It is not surprising that when the ice thickness, bed elevation, sliding velocity, or water thickness fields are highly-variable in space then we can expect larger speeds $|\bV|$ in steady state, and formula \eqref{eq:qabstract} reflects this intuition.  The magnitude of the velocity determines the CFL time step restriction \citep{MortonMayers}, so spatial variability will generally shorten time steps; see section \ref{sec:num}.


\subsection{Hydraulic conductivity in steady state}  In steady state the water amount $W$ roughly scales with $1/k$ where $k$ is the hydraulic conductivity.  In fact, if we combine equation \eqref{eq:masscontsteady} with \eqref{eq:qabstract} and rearrange slightly then we find
\begin{equation}
