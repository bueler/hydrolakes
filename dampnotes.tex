...
\end{equation}
This scheme is also called the ``donor cell'' upwind method \citep{LeVeque}.  In \eqref{eq:upwindfirst} we suppose that the upwinded flux at $x_{i+1/2}=x_e$ uses $u_i$ because $v_e\ge 0$.
...
\begin{equation}
Q_e = v_e \left[u_{i+1} + \psi\left((\theta_{i+1})^{-1}\right) (u_i - u_{i+1})\right], \qquad v_e < 0. \label{eq:fluxlimiterform}
\end{equation}
