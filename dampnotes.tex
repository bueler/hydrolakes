...
\subsection*{Flux discretization}  Because \eqref{eq:adeqn} is an advection-dominated evolution PDE, the well-known goals for discretizing the fluxes include both reduced truncation error and also non-oscillation and positivity \citep{HundsdorferVerwer2010}.  Two alternative schemes addressing these goals are described in this section, one simpler, linear, and low-order and the other more-complicated, non-linear, and higher-order.  These schemes, which are often associated to finite volume discretization \citep{LeVeque}, are introduced using an advection-only model equation.

Consider the model equation
\begin{equation} \label{eq:modeladvect}
u_t + (v(x) u)_x = 0
\end{equation}
for some quantity $u(t,x)$ transported by a flux $Q = v(x) u$.  Though we allow the velocity $v(x)$ to vary spatially in this discussion, let us assume that $v(x)\ge 0$.  Suppose that a grid point $x_i$ is the center of a cell and consider the flux at the cell interfaces $x_{i-1/2}=x_e$ and $x_{i+1/2}=x_w$.  Suppose we discretize space but not time so that $u_i=u_i(t)$, and denote time derivatives by dot: $\dot u_i = du_i/dt$.
