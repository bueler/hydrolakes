...
\medskip
\caption{(a) The steady state function $P(W)$ equation \eqref{eq:PofWsteady} depends on the sliding speed $|\bv_b|$; four cases are shown (solid curves) using a fixed uniform ice thickness of $H=1000$ m.  Relation \eqref{eq:PofWFC} is not used in the current work (dashed black; shown with $W_{\text{crit}}=0.5$ m for comparison).  (b)  The same function $P(W)$ depends on overburden pressure $P_o=\rho_i g H$.  We fix $|\bv_b|=100$ m/a and consider four cases of uniform thickness $H$.}
\label{fig:psteady}
\end{figure}
