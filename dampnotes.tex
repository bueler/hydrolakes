...
\subsection*{Flux discretization}  Because \eqref{eq:adeqn} is an advection-dominated PDE, well-known goals for discretizing the fluxes include non-oscillation and/or positivity properties \citep{HundsdorferVerwer2010} in addition to reduced truncation error.  Schemes addressing these goals, which are often described as finite volume discretizations \citep{LeVeque}, are introduced here using an advection-only model equation.

Consider the model equation
\begin{equation} \label{eq:modeladvect}
u_t + (v(x) u)_x = 0
\end{equation}
in which some quantity $u(t,x)$ is transported by a flux $Q = v(x) u$ with velocity $v(x)$.  We allow the velocity $v(x)$ to vary spatially but we assume that $v(x)\ge 0$.  Suppose that grid points $x_i$ are the centers of a cell, that there is equal spacing $\Delta x$, and that we denote cell interfaces $x_{i-1/2}=x_w$ and $x_{i+1/2}=x_e$.  Suppose we do not discretize time: denote $u_i=u_i(t)$ and time derivatives $\dot u_i = du_i/dt$.

We say a discretization of \eqref{eq:modeladvect} is \emph{positive} if $u(0,x)\ge 0$ for all $x$ implies $u(t,x)\ge 0$ for all $t\ge 0$ and all $x$.  Of course, such a property is desirable for any conservation scheme for the water thickness $W\ge 0$.  The basic discretization of \eqref{eq:modeladvect} is
\begin{equation}
\dot u_i + \frac{Q_e - Q_w}{\Delta x} = 0, \label{eq:basicmodelFD}
\end{equation}
but we must choose a flux parameterization.  The simplest positive scheme for the fluxes is first-order upwinding in conservative (flux) form \citep[section I.4.3]{HundsdorferVerwer2010}:
\begin{equation}
Q_e = v_e u_i \label{eq:upwindfluxfirst}
\end{equation}
This scheme is also called the ``donor cell'' upwind method \citep{LeVeque}.  In \eqref{eq:upwindfluxfirst} we suppose that the upwinded flux at $x_{i+1/2}=x_e$ uses $u_i$ because $v_e\ge 0$.

A second-order truncation error scheme which is \emph{not} positive uses centered differences for the flux.  This adds a correction to the first-order upwind form:
\begin{equation}
Q_e = v_e \frac{u_i+u_{i+1}}{2} = v_e \left[u_i + \frac{1}{2} (u_{i+1} - u_i)\right]. \label{eq:centerfluxfirst}
\end{equation}
This scheme, which can be stable when the time-discretization is implicit, causes oscillations and is not positive.  Another such scheme is third-order upwind-biased fluxes which can again be written as a correction to first-order upwinding:
\begin{equation}
...
\end{equation}
Unfortunately, despite the upwind-biasing, this scheme is also oscillatory and non-positive.

The approach of flux-limiting is that the corrections in \eqref{eq:centerfluxfirst} and \eqref{eq:thirdfluxfirst} are too large if we seek to preserve positivity \citep[section III.1.1]{HundsdorferVerwer2010}.  That is, the correction is too large near local extrema.  Unfortunately, Godunov's barrier theorem \citep[section I.7.1]{HundsdorferVerwer2010} says that we cannot get truncation error better than first-order with a discretization that is both linear and positive.  Fortunately, nonlinear ``flux-limited'' (correction-limited) formulas with these properties do exist.  Because the schemes under consideration are explicit, such nonlinearity does not represent a significant computational cost.
...
\end{equation}
If in the model equation we have $v(x)<0$ then the same function $\psi$ should be used but with the direction of differencing reversed \citep[section III.1.1]{HundsdorferVerwer2010}:
\begin{equation}
Q_e = v_e \left[u_{i+1} + \psi\left((\theta_{i+1})^{-1}\right) (u_i - u_{i+1})\right], \qquad v_e < 0. \label{eq:fluxlimiterformreversed}
\end{equation}
The forms for $Q_w$ in \eqref{eq:basicmodelFD} are the same as these but with $Q_e\to Q_w$, $v_e \to v_w$, $i\to i-1$, and $i+1\to i$.

Table \ref{tab:fluxlimiters} shows several cases for $\psi$ including those already considered.  Note that $\theta_i$ in \eqref{eq:thetadefine} can take any real value.  For the time-discretizations we will consider, sufficient conditions on $\psi(\theta)$ to give a positive advection scheme for model equation \eqref{eq:modeladvect} are
\begin{equation}
...
From \eqref{eq:thetadefine} we see that for smooth functions and fine grids we have $\theta_i\approx 1$ except near extrema where $u_{i+1} - u_i$ and $u_i - u_{i-1}$ are both near zero.  The Koren flux-limiter in Table \ref{tab:fluxlimiters}, which we will use in practice, has $\psi(\theta) = \frac{1}{3}+\frac{1}{6} \theta$ for $\frac{2}{5} \le \theta \le 4$.  We can think of the resulting scheme as a positive form of the third-order upwind-biased scheme.
...
\hline
    $\ast$ first-order conservative upwinding  & $\phantom{\Big|}\psi(\theta) = 0$ \\
    ...
    $\ast$ Koren                               & $\phantom{\Big|}\psi(\theta) = \max\left\{0,\min\{1,\theta,\frac{1}{3}+\frac{1}{6} \theta\}\right\}$  \\
    $\ast$ van Leer                            & $\phantom{\Big|}\psi(\theta) = \frac{1}{2} \frac{\theta + |\theta|}{1+\theta}$  \\
    \hline
  \end{tabular}
 \label{tab:fluxlimiters}
\end{table}

To summarize our understanding of our schemes for the model equation \eqref{eq:modeladvect}, first-order upwinding \eqref{eq:upwindfluxfirst} gives positivity but only $O(\Delta x^1)$ truncation error.  Second-order centered differencing \eqref{eq:centerfluxfirst} and third-order upwind-biased differencing \eqref{eq:thirdfluxfirst} give better truncation error (i.e.~$O(\Delta x^2)$ and $O(\Delta x^3)$, respectively), but not positivity.  The flux-limited Koren and van Leer schemes in Table \ref{tab:fluxlimiters} give positivity and also better truncation error away from difficult areas (i.e.~near extrema and non-smooth areas where $\theta_j$ is not near one), but they revert to first-order in these difficult areas.

FIXME: address time-stepping

FIXME: actually give formulas which apply to \eqref{eq:Wfd}

Even for one of the positive advection schemes described here, and even if the combined advection-diffusion scheme is still positive as is proved in the Appendix for first-order upwinded fluxes, if the water input $\Phi$ is negative then we must actively enforce the positivity of the water thickness $W$.  That is, positivity of the advection and diffusion parts is good but it does not ensure positivity of the solution if there is actual water removal ($\Phi < 0$).
