...
We say a discretization of \eqref{eq:modeladvect} is \emph{positive} if $u(0,x)\ge 0$ for all $x$ implies $u(t,x)\ge 0$ for all $t\ge 0$ and all $x$.  Of course, such a property is desirable for any conservation scheme for the water thickness $W\ge 0$.  The simplest positive scheme for \eqref{eq:modeladvect} is first-order upwinding in conservative (flux) form \citep[section I.4.3]{HundsdorferVerwer2010}:
\begin{equation}
\dot u_i + \frac{v_e u_i - v_w u_{i-1}}{\Delta x} = 0 \label{eq:upwindfirst}
\end{equation}
where $v_e = v(x_e)$ and $v_w = v(x_w)$.  This scheme is also called the ``donor cell'' upwind method \citep{LeVeque}.  In \eqref{eq:upwindfirst} we suppose that the upwinded flux at $x_{i+1/2}=x_e$ uses $u_i$ because $v_e\ge 0$:
\begin{equation}
Q_e = v_e u_i. \label{eq:upwindfluxfirst}
\end{equation}
A second-order truncation error, but \emph{not} positive, scheme is to use centered fluxes.  This adds a correction to the first-order upwind form:
\begin{equation}
...
The approach of flux-limiting is that the corrections in \eqref{eq:centerfluxfirst} and \eqref{eq:thirdfluxfirst} are too large, especially near local extrema and rapid variation areas, if we seek to preserve positivity \citep[section III.1.1]{HundsdorferVerwer2010}.  Unfortunately, Godunov's barrier theorem \citep[section I.7.1]{HundsdorferVerwer2010} says that we cannot get truncation error better than first-order with a discretization that is both linear and positive, but it nonlinear forms do exist.  Because the schemes under consideration are explicit, such nonlinearity does not represent a significant computational cost.
...
\begin{equation}
Q_e = v_e \left[u_i + \psi(\theta_i) (u_{i+1} - u_i)\right]. \label{eq:fluxlimiterform}
\end{equation}
...
\hline
    ...
    $\ast$ \cite{Koren1993}                   & $\phantom{\Big|}\psi(\theta) = \max\left\{0,\min\{1,\frac{1}{3}+\frac{1}{6} \theta\}\right\}$  \\
    $\ast$ \cite{vanLeer1974}                 & $\phantom{\Big|}\psi(\theta) = \frac{1}{2} \frac{\theta + |\theta|}{1+\theta}$  \\
    \hline
  \end{tabular}
 \label{tab:fluxlimiters}
\end{table}

FIXME: compare 1stupwind = "donor cell", centered, flux-limited a la H\&V page 216

FIXME: first-order upwinding gives low order $O(\Delta t^1 + \Delta x^1 + \Delta y^1)$ truncation error and flux-limited is better but still low order near extrema of $u(x)$
