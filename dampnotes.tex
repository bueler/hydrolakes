...
\bigskip
\caption{The function $P(W)$ in steady state has different shapes depending on the size of the sliding velocity.  If there is sliding $|\bv_b|>0$ then $\partial P / \partial W = \infty$ as water amount reaches the roughness scale $W=W_r$.  (In this case $W_r=1$ m and $c_1/(c_2 A) = 3.94\times 10^{24}\,\text{Pa}^3\,\text{s}\,\text{m}^{-1}$.)}
\label{fig:psteady}
\end{figure}
