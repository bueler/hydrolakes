...
\medskip
\caption{Function $K(W)$ defined by \eqref{eq:nontrivialK} (solid) has a bounded derivative but also has an abrupt change in behavior at $W=W_r$.  The constant case $K(W)=K_0$ is also shown (dashed).  Parameters $W_r=1,K_0=0.01,K_1=0.1$ are chosen for illustration.}
\label{fig:Kcompare}
\end{figure}
