...
a scaled basal sliding speed which has units of pressure.  Then \eqref{eq:steadyboundfirst} is equivalent to
\begin{equation}
W \ge W_c := \frac{s_b^3 W_r - P_o^3 Y_0}{s_b^3 + P_o^3} \qquad \text{ in steady state}, \label{eq:steadyboundsecond}
\end{equation}
which says that the water amount is above a critical level that depends on the sliding and the overburden pressure.  (Inequality \eqref{eq:steadyboundfirst} is actually less restrictive in the regularized case $Y_0>0$ than in the unregularized case $Y_0=0$.)  If \eqref{eq:steadyboundsecond} holds then
\begin{equation}
...
Figure \ref{fig:psteady-vb} shows the function $P(W)$ from \eqref{eq:PofWsteady} for several cases of uniform sliding speed $|\bv_b|$.  Figure \ref{fig:psteady-Po} shows several cases of of uniform overburden pressure $P_o$.  (Replacing $Y_0=0$ makes no apparent difference in these figures; not shown.)  We see that as the water amount reaches the roughness scale ($W\upto W_r=1$ m) the pressure rises rapidly to overburden ($P(W) \upto P_o$).  At the other extreme, we see that $P(W) \downto 0$ if $W \downto W_c$, and that if $|\bv_b|$ is modest then this occurs for $W$ (i.e.~$W_c \ll W_r$).  The curves in these figures, which describe steady state, do not include the interval of water thicknesses less than the critical thickness $W_c$ defined in \eqref{eq:steadyboundfirst} (i.e.~$0\le W < W_c$), underpressure conditions which are not achievable in steady state.

\begin{figure}[ht]
\includegraphics[width=3.5in,keepaspectratio=true]{figs/psteady-vb}
\medskip
\caption{The steady state function $P(W)$ defined by equation \eqref{eq:PofWsteady} depends on the sliding speed.  Four cases are shown using a fixed uniform ice thickness of $H=1000$ m: $|\bv_b|=0$ m/a (blue), $|\bv_b|=10$ m/a (green), $|\bv_b|=100$ m/a (red), and $|\bv_b|=1000$ m/a (cyan).  The values of $W_c$ for these cases are indicated by black dots with $P(W_c)=0$.  Relation \eqref{eq:PofWFC} is shown with $W_{\text{crit}}=W_r$ for comparison (dashed black).}
...
\medskip
\caption{Function $P(W)$ defined by \eqref{eq:PofWsteady} also depends on overburden pressure $P_o=\rho_i g H$.  We fix $|\bv_b|=100$ m/a and consider four cases of uniform thickness $H$ as shown.}
...
\end{equation}
This is used to describe the water pressure under glaciers including the Trapridge glacier in Canada.  On the one hand their formula has features in common with equation \eqref{eq:PofWsteady}: both functions are increasing and both relate the water pressure to the overburden pressure $P_o$.  However, while in \eqref{eq:PofWsteady} the relation to $P_o$ is additive, in \eqref{eq:PofWFC} it is a multiplicative scaling.  The power law form of $P_{FC}(W)$ is not justified by the physical reasoning which led to equation \eqref{eq:PofWsteady}, even in steady state.   It would appear that any functional relationship $P(W)$ should also depend on the sliding velocity if cavitation is an influence on the water pressure.  For example, in the small sliding cases (e.g.~$|\bv_b| \ll 10$ m/a) creep will force $P(W) \approx P_o$ for even a modest amount of water.  Of course the $W>W_{\text{crit}}$ case gives $P_{FC}(W) > P_o$, as noted by \cite{Schoofetal2012}; this physically-inconsistent state that does not arise in \eqref{eq:PofWsteady}.  Perhaps the most important contrast between \cite{FlowersClarke2002_theory} and the current paper is that we have no functional relationship $P=P(W)$ in the nonsteady theory.
