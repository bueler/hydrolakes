...
\frac{E_0}{P_o} \frac{\partial P}{\partial t} &= \Div \left(\frac{K\,W}{\rho_w g} \grad P \right) + c_2 A |P_o - P|^3 W \label{eq:ALTdiffusionpressure} \\
  &\qquad + \Div \left(K\,W \grad b \right) - c_1 |\bv_b| \min\left\{0,W_r - W\right\} \notag \\
  &\qquad + \Div \left(K\,W \grad W \right) + \Phi, \notag
\end{align}
...
where $V=(\alpha,\beta)$ is the advecting velocity, for the upwind part of the scheme to maintain stability.  The time step must also satisfy
    $$\Delta t\, (2 K \max W) \left(\frac{1}{\Delta x^2} + \frac{1}{\Delta y^2}\right) \le \frac{1}{2}$$
for the diffusion scheme to be stable.  To give some practical values, we are interested in grids of size $\Delta x = \Delta y = 500$ m.  The the maximum speed $|V|$ is about $10^5$ m/a in trial runs of the model, so $\max |\alpha| = \max |\beta| \approx 0.002$ m/s.  We take $K=10^{-2}$ m/s and $\max W=1$ m as representative values.  In that case the advective restriction gives about $\Delta t \le 0.001$ year while the diffusive restriction is about $\Delta t \le 0.1$ year.  Thus, unless velocities are unusually slow and deep lakes develop (so that $KW$ is large), we should not worry so much about the diffusive time scale.  The problem is primarily an advection.
...
\subsection*{Numerical scheme for alternative nearly-full equation \eqref{eq:ALTdiffusionpressure}}   FIXME:  start by computing time step restriction  
