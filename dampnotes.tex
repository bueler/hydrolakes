...
Finally we can state the two alternatives we will test as discrete flux schemes in equation \eqref{eq:Wfd}.  Recall that the flux at the staggered grid location $(x_{i+1/2},y_j)$ is denoted ``$Q_e(\alpha_e)$'' and the flux at $(x_i,y_{j+1/2})$ is denoted ``$Q_n(\beta_n)$.''  Recall also that the velocity has components $\bV=(\alpha,\beta)$ which are evaluated at these staggered locations following \eqref{eq:velocitycomp}.  To ensure conservation we must have a single formula for $Q_{i+1/2,j}$ whether this flux is ``$Q_e$'' for $(x_i,y_j)$ or ``$Q_w$'' for $(x_{i+1},y_j)$; similar comments apply to ``$Q_n$'' versus ``$Q_s$''.  Thus we simply give formulas for $Q_e$ and $Q_n$:
\begin{align}
...
\end{align*}
Clearly one need not compute these ``$\theta$s'' when using first-order upwind.  On the other hand, when using the Koren or van Leer forms the stencil in Figure \ref{fig:stencil} is incomplete; the additional regular grid neighbors $W_{i+2,j}$, $W_{i-2,j}$, $W_{i,j+2}$, $W_{i,j-2}$ are each (potentially) involved in updating $W_{i,j}$.

For either one of the positive advection schemes compared below, and even if the combined advection-diffusion scheme is still positive as is proved in the Appendix for first-order upwinded fluxes, if the water input $\Phi$ is negative then we must actively enforce the positivity of the water thickness $W$.  That is, positivity of the advection and diffusion schemes is a highly-desirable property but it does not ensure positivity of the solution if there is actual water removal ($\Phi < 0$), and so we must project (reset) $W$ to be nonnegative at the end of each time step anyway.
...
\subsection*{Verification of the coupled model}  By using the steady-state nearly-exact solution constructed in section \ref{sec:steadyverif} we can now verify the numerical schemes described in section \ref{sec:num}.  Verification refers to the process of measuring the errors made by the numerical scheme, relative to the exact solution, especially as the numerical grid is refined.  In this case our exact solution is for the steady-state so we initialize our time-stepping numerical scheme with the exact steady solution and we measure the error relative to the steady exact values after some period of time integration.  That is, we measure the drift away from the exact steady solution, and we take the magnitude of that drift to depend on the spatial grid size.

We use the constants in Table \ref{tab:verifconstants}.  The exact solution is shown in Figures \ref{fig:Wexact} and \ref{fig:Pexact}.  We do a one month run on grids with resolutions decreasing by factors of two from $6$ km to $375$ m.  The results are shown in Figures \ref{fig:refineW} and \ref{fig:refineP}.
...
These results suggest that our numerical solution method for these coupled advection-diffusion (for $W$) and diffusion-reaction (for $P$) equations are being solved correctly; we have evidence for numerical convergence.  However the rate of convergence is not great.  Note that the location of the large errors is entirely in the neighborhood of the ice margin $r=L$.  The method for handling boundary conditions is critical to determining the magnitude of the error and its rate of decay.  Further research is needed to decide how to handle this boundary in a more-nearly optimal manner.
