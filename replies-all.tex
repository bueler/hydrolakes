...
\indent In fact, equation (55) is used as stated at all grid points, regardless of neighbor mask state.\footnote{See method \texttt{raw\_update\_W()} in file \texttt{PISMRoutingHydrology.cc} in branch \texttt{stable0.6} of the PISM source code.}  Thus the boundary conditions, which can all be interpreted as free boundary conditions and which are motivated by concerns listed in replies above, are applied in steps (ii), (vii), (viii), (ix), and (x).}
