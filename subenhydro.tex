...
In addition to laterally-transportable water $W$ there is water stored locally in the till.  The amount of this water is described by a effective thickness $\Wtil(t,x,y)$.  The limited storage capacity of the till gives an inequality $\Wtil \le \Wtilmax$.  Though $\Wtilmax$ could vary in time and space according to a sediment evolution model, in the current paper it is a fixed nonnegative constant \citep{BBssasliding}.  Note $\Wtilmax=0$ is allowed, and it corresponds to removing the till storage mechanism.  In section \ref{sec:capacity} we give a model for transfer $W \leftrightarrow \Wtil$.

We also model englacial water storage in a system of crevasses, cracks, veins, and/or moulins of unspecified morphology but with constant macroporosity $\phi$ \citep{Bartholomausetal2011}.  Throughout this paper the case $\phi=0$ of no englacial storage is also allowed; it may apply to cold glaciers and the interior of ice sheets.  The extension that $\phi$ varies in time and map-plane location is natural but it is not specifically considered here.

The mass of englacial water can be tracked with the elevation above the bed $z_{en}(t,x,y)$ for the top of the (notional) ``water table'' in the porous glacier.  In a column of the glacier with cross-sectional area $\Delta x \times \Delta y = \Delta a$ the volume of water is then $\phi\, \Delta a \, z_{en}$ \citep{Bartholomausetal2011}.  We can just as easily track the water by an effective thickness for the englacial water, however, and we define
\begin{equation}
...
Suppose $\bq$ is the (vector) water flux (units $\text{m}^2\,\text{s}^{-1}$).  We will not consider the possibility of lateral englacial transport; the water flux $\bq$ is concentrated within a subglacial layer.

The total effective thickness of the water at map-plane location $(x,y)$ and time $t$ is
\begin{equation}
\Wtot = W + \Wtil + \Wen.  \label{eq:totalwater}
\end{equation}
We always assume the water thicknesses are nonnegative: $W \ge 0$, $\Wtil \ge 0$, and $\Wen \ge 0$.  In two spatial dimensions the mass conservation equation is \citep{Clarke05}
\begin{equation} \label{eq:conserve}
\frac{\partial \Wtot}{\partial t} + \Div \bq = \frac{m}{\rho_w}
\end{equation}
where $\rho_w$ is the density of fresh liquid water.  Coupling between the subglacial and englacial systems, and transfer to-and-from till storage, is addressed in section \ref{sec:capacity}.
...
\subsection*{Dynamics of local till storage}
...
\medskip
\caption{The till-stored water amount $\Wtil$ evolves by \eqref{eq:tilldynamics}.  If $W < \Wtilmax$ then $\Wtil$ approaches $W$, while if $W > \Wtilmax$ then $\Wtil$ approaches $\Wtilmax$.}
...
The problem considered in the main paper (FIXME: sections?) includes mass conservation equation \eqref{eq:conserve} and equation (FIXME) for the evolution of the water stored locally in till pores.  An abstract view of this problem is the following pair of equations for abstract quantities $u(t,x,y),v(t,x,y)$ satisfying bounds $u\ge 0$ and $0 \le v \le v_{\text{max}}$:
\begin{align}
...
so that the discrete evolution of $v$ maintains the constraint (requirement) that the storage capacity of the till is not exceeded.
