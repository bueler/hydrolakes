...
\psi &= P + \rho_w g (b + W), \notag \\
\bV &= - \frac{k W^{\alpha-1}}{\rho_w g} \grad\left(P + \rho_w g b\right), \notag \\
\frac{\phi+\phi_0}{\rho_w g} \frac{\partial P}{\partial t} &= \Div \left(\frac{k W^\alpha}{\rho_w g}\, \grad \psi \right) + \frac{m+S}{\rho_w} \label{eq:bluebox} \\
...
\phantom{dsaf} \frac{\partial W}{\partial t} &= - \Div\left(\bV\, W\right) + \Div \left(k W^\alpha \grad W\right) + \frac{m+S}{\rho_w}. \notag
\end{empheq}
...
\phantom{ldsfj} \frac{\partial W}{\partial t} &= - \Div\left(\tilde\bV\, W\right) + \Div \left(k W^\alpha \grad W\right) + \frac{m+S}{\rho_w}. \phantom{ldsfj} \notag
\end{empheq}
...
\end{equation}
In this case we can compute the ``west'' and ``south'' values by shifting indices: $W_w = W_e\big|_{(i-1,j)}$ and $W_s = W_n\big|_{(i,j-1)}$.  Thus there are only two distinct staggered grid values (i.e.~east and north) to compute per regular grid location $(x_i,y_j)$.  We get the velocity components needed at staggered locations by differencing:
\begin{align}
