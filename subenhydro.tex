...
\end{equation}
This makes no change in the $\beta=2$ case.  Now define the nonlinear effective hydraulic conductivity
\begin{equation}
K = K(W,\grad P,\grad b) = k W^{\alpha-1} \left|\grad(P+\rho_w g b)\right|^{\beta - 2}. \label{eq:Kdefine}
\end{equation}
Note $K = k$ in the $\alpha=1$ and $\beta=2$ case covered by \eqref{eq:fluxearly}.  In terms of this nonlinear conductivity we write the transporting velocity field and the nonlinear diffusivity as
\begin{equation} \label{eq:vexpression}
  \bV = - K\, \grad \left(P + \rho_w g b\right), \qquad D = \rho_w g K W.
\end{equation}
Then expression \eqref{eq:firstfluxdecomp} is replaced by a clear advection-diffusion decomposition of the flux,
...
D &= \rho_w g K W, \label{eq:bluebox} \\
