...
\begin{equation}
  \bq = - \frac{K(W)\, W}{\rho_w g} \grad \left(P + \rho_w g b\right) - K(W) W \grad W. \label{eq:firstfluxdecomp}
\end{equation}
...
\begin{equation} \label{eq:vexpression}
  \bV = - \frac{K(W)}{\rho_w g} \grad \left(P + \rho_w g b\right).
\end{equation}
...
\begin{equation} \label{eq:adeqn}
  \frac{\partial W}{\partial t} + \Div\left(\bV\, W\right) = \Div \left(K(W) W \grad W\right) + \left[\frac{m+S}{\rho_w} - \phi \frac{\partial z_{en}}{\partial t}\right].
\end{equation}
(The left side is written as a material derivative $d(\cdot)/dt = \partial(\cdot)/\partial t + \Div\left(\bV\,\cdot\right)$.)

The term in square brackets in \eqref{eq:adeqn} groups all the mass inputs into the subglacial layer.  Part of this input, that which is proportional to the rate at which the englacial water level falls (i.e.~$-\partial z_{en}/\partial t$), is from the efficient connection to the macroporous englacial storage.  This term can be of either sign in a dynamically-evolving manner.  The total input into the subglacial layer is a sum of contributions from wall melt $m$, supraglacial input $S$ (through an unmodeled mechanism), and this connection to englacial storage.

There are different numerical schemes for the advection term $\Div\left(\bV\, W\right)$ and the diffusion term $\Div \left(K(W) W \grad W\right)$ in \eqref{eq:adeqn}; see section \ref{sec:num}.  These different schemes impose time step restrictions of different magnitudes.  We will see in practice that equation \eqref{eq:adeqn} is advection-dominated.  However, in near-steady conditions the velocity $\bV$ is nearly proportional to $-\grad W$ so the numerical schemes for advection and for diffusion must be tested in combination.  We measure the convergence behavior of the combined schemes in section \ref{sec:results}.
...
\begin{gather}
  \frac{\partial W}{\partial t} + \Div\left(\tilde\bV\, W\right) = \Div\left(K_0 \,W\, \grad W\right) + \frac{m+S}{\rho_w}   \label{eq:PisoverConservation} \\
  \text{where} \qquad \tilde\bV = - K_0 \left[\frac{\rho_i}{\rho_w} \grad h + \left(1-\frac{\rho_i}{\rho_w}\right) \grad b\right]. \notag
\end{gather}
...
Using \eqref{eq:Penhydrostatic} we may rewrite the advection-diffusion conservation equation \eqref{eq:adeqn} as
\begin{equation} \label{eq:conservewithPen}
\frac{\partial W}{\partial t} + \Div\left(\bV\, W\right) = \Div \left(K(W) W \grad W\right) + \left[\frac{m+S}{\rho_w} - \frac{\phi}{\rho_w g}\frac{\partial P}{\partial t}\right].
\end{equation}
This new form allows us to eliminate $z_{en}$ and/or $W_{en}$ as a model state variable.  We see that the rate of water transfer from the subglacial system to the englacial system, $(\phi/(\rho_w g)) \partial P/\partial t$ in the above equation, may be thought of as a source term in the mass conservation equation.
...
0 &\le P \le P_o, \notag \\
\psi &= P + \rho_w g (b + W), \notag \\
\bV &= - \frac{K(W)}{\rho_w g} \grad\left(P + \rho_w g b\right), \label{eq:bluebox} \\
\frac{\phi}{\rho_w g} \frac{\partial P}{\partial t} &= \Div \left(\frac{K(W)\,W}{\rho_w g}\, \grad \psi \right) + \frac{m+S}{\rho_w} \notag \\
  &\qquad \qquad + c_2 A (P_o - P)^3 (W+Y_0) - c_1 |\bv_b| (W_r - W)_+, \notag \\
\frac{\partial W}{\partial t} + \Div\left(\bV\, W\right) &= \Div \left(K(W)\, W \grad W\right) + \left[\frac{m+S}{\rho_w} - \frac{\phi}{\rho_w g}\frac{\partial P}{\partial t}\right]. \notag
\end{empheq}
This form of the mathematical model is written so as to allow three particular limits to be clear:
\begin{itemize}
\item The $\phi\to 0$ limit is the $\alpha=1,\beta=1$ form of the model described by \cite{Schoofetal2012}.  The equations in this case are straightforward to write down: we simply replace the term ``$(\phi/(\rho_w g)) \partial P / \partial t$'' by zero.  Said the other way, the above model \eqref{eq:bluebox} is a parabolic regularization of the \cite{Schoofetal2012} model based on efficient connection to porous englacial storage, with small porosity parameter $\phi \ll 1$.
\item The $P \to P_o$ limit is the standard model for locating subglacial lakes under cold (i.e.~also $\phi \to 0$) ice sheets \cite{Siegertetal2009}.  (See the discussion of equation \eqref{eq:PisoverConservation} in section \ref{sec:closures}.)  In this limit the pressure is not an unknown and the parameterization of cavity evolution is inactive:
\begin{empheq}[box=\mybluebox]{align}
W &\ge 0, \notag \\
\bV &= - \frac{K(W)}{\rho_w g} \grad\left(P_o + \rho_w g b\right), \label{eq:lakesmodel} \\
\phantom{ldsfj} \frac{\partial W}{\partial t} + \Div\left(\bV\, W\right) &= \Div \left(K(W)\, W \grad W\right) + \left[\frac{m+S}{\rho_w}\right]. \phantom{ldsfj} \notag
\end{empheq}
