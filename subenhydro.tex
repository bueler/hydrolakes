...
\newcommand{\Wtl}{W_{\text{tl}}}
\newcommand{\Wtlmax}{W_{\text{tl}}^{\text{max}}}
\newcommand{\Wen}{W_{\text{en}}}

FIXME: add text
\begin{equation}
\frac{\partial \Wtl}{\partial t} = \mu \left(\min\left\{W,\Wtlmax\right\} - \Wtl\right) \label{eq:tilldynamics}
\end{equation}
FIXME: add text
...
\medskip
\caption{The till-stored water amount $\Wtl$ evolves by \eqref{eq:tilldynamics}.  If $W < \Wtlmax$ then $\Wtl$ approaches $W$, while if $W > \Wtlmax$ then $\Wtl$ approaches $\Wtlmax$.}
