...
At this point we have a complete new mathematical model, consisting of mass conservation equation \eqref{eq:adeqn} for evolution of $W$, equation \eqref{eq:pressureequation} for evolution of pressure $P$, equation \eqref{eq:ratePenWen} which allows $W_{en}$ to follow $P$, and the bounds on $W$ and $P$:
\begin{empheq}[box=\mybluebox]{align}
W &\ge 0, \notag \\
0 &\le P \le P_o, \notag \\
\psi &= P + \rho_w g (b + W), \notag \\
\bV &= - \frac{k W^{\alpha-1}}{\rho_w g} \grad\left(P + \rho_w g b\right), \notag \\
\frac{\phi}{\rho_w g} \frac{\partial P}{\partial t} &= \Div \left(\frac{k W^\alpha}{\rho_w g}\, \grad \psi \right) + \frac{m+S}{\rho_w} \label{eq:bluebox} \\
  &\qquad \qquad + c_2 A (P_o - P)^3 (W+Y_0) - c_1 |\bv_b| (W_r - W)_+, \notag \\
\frac{\partial W_{en}}{\partial t} &= \frac{\phi}{\rho_w g} \frac{\partial P}{\partial t}, \notag \\
 \frac{\partial W}{\partial t} + \Div\left(\bV\, W\right) &= \Div \left(k W^\alpha \grad W\right) + \left[\frac{m+S}{\rho_w} - \frac{\partial W_{en}}{\partial t}\right]. \notag
\end{empheq}
This mathematical model is written so as to allow three limits to be clear:
\begin{itemize}

\item The $\phi\to 0$ limit of \eqref{eq:bluebox} is the $\beta=2$ form of the model described by \cite{Schoofetal2012}.  The equations in this case are straightforward to write down.  We replace the term $(\phi/(\rho_w g)) \partial P / \partial t = \partial W_{en}/\partial t$ by zero, and we set $Y_0=0$ to get complete agreement:
\begin{empheq}[box=\mybluebox]{align}
W &\ge 0, \notag \\
0 &\le P \le P_o, \notag \\
\psi &= P + \rho_w g (b + W), \notag \\
\bV &= - \frac{k W^{\alpha-1}}{\rho_w g} \grad\left(P + \rho_w g b\right),  \label{eq:schoofsmodel} \\
0 &= \Div \left(\frac{k W^\alpha}{\rho_w g}\, \grad \psi \right) + \frac{m+S}{\rho_w} + c_2 A (P_o - P)^3 W - c_1 |\bv_b| (W_r - W)_+, \phantom{dsaf} \notag \\
\phantom{dsaf} \frac{\partial W}{\partial t} + &\Div\left(\bV\, W\right) = \Div \left(k W^\alpha \grad W\right) + \left[\frac{m+S}{\rho_w}\right]. \notag
\end{empheq}
Physically, the $\phi \to 0$ limit of \eqref{eq:bluebox} gives model \eqref{eq:schoofsmodel} by removing the englacial storage.  Said mathematically, our model \eqref{eq:bluebox} is a parabolic regularization of \eqref{eq:schoofsmodel}, the \cite{Schoofetal2012} model, based on efficient connection to porous englacial storage with a small porosity parameter $\phi \ll 1$.

\item The $P \to P_o$ limit of \eqref{eq:bluebox}, in which physical processes for the evolution of pressure are ignored and the pressure reverts to the overburden value $P_o$, is the standard model for locating subglacial lakes under cold (i.e.~also $\phi \to 0$) ice sheets \cite{Siegertetal2009}.  In this limit the pressure is not an unknown and the parameterization of cavity evolution is inactive:
\begin{empheq}[box=\mybluebox]{align}
W &\ge 0, \notag \\
\tilde\bV &= - \frac{k W^{\alpha-1}}{\rho_w g} \grad\left(P_o + \rho_w g b\right), \label{eq:lakesmodel} \\
\phantom{ldsfj} \frac{\partial W}{\partial t} + \Div\left(\tilde\bV\, W\right) &= \Div \left(k W^\alpha \grad W\right) + \left[\frac{m+S}{\rho_w}\right]. \phantom{ldsfj} \notag
\end{empheq}
As noted in section \ref{sec:closures}, in the $\alpha=1$ case this simplified model routes water according to a velocity $\tilde\bV$ which is determined entirely by ice geometry.
...
\emph{scalar parameters} & $A$, $\alpha$, $c_1$, $c_2$, $\phi$, $k$, $W_r$, $Y_0$ \\
...
\hline
    ...
    $\phi$ & $0.01$ & & macroporosity of englacial system \\
...
\begin{align}
\alpha_e &= - \frac{k W_e^{\alpha-1}}{\rho_w g} \frac{P_{i+1,j}-P_{i,j}}{\Delta x} - k W_e^{\alpha-1} \frac{b_{i+1,j}-b_{i,j}}{\Delta x}, \label{eq:velocitycomp} \\
\beta_n  &= - \frac{k W_n^{\alpha-1}}{\rho_w g} \frac{P_{i,j+1}-P_{i,j}}{\Delta y} - k W_n^{\alpha-1} \frac{b_{i,j+1}-b_{i,j}}{\Delta y}. \notag
\end{align}
...
\begin{align}
 &\frac{W_{i,j}^{l+1} - \Wlij}{\Delta t} + \frac{Q_e(\alpha_e) - Q_w(\alpha_w)}{\Delta x} + \frac{Q_n(\beta_n) - Q_s(\beta_s)}{\Delta y}  \label{eq:Wfd} \\
      &\qquad = \frac{k W_e^\alpha \left(W_{i+1,j}^l - \Wlij\right) - k W_w^\alpha \left(\Wlij - W_{i-1,j}^l\right)}{\Delta x^2}  \notag \\
      &\qquad\qquad\qquad + \frac{k W_n^\alpha \left(W_{i,j+1}^l - \Wlij\right) - k W_s^\alpha \left(\Wlij - W_{i,j-1}^l\right)}{\Delta y^2} \notag \\
      &\qquad\qquad + \left[\frac{m_{ij}+S_{ij}}{\rho_w} - \left\{\frac{\partial W_{en}}{\partial t}\right\}_{ij}\right]. \notag
\end{align}
The values $m_{ij}$ and $S_{ij}$ are supplied by other components of an ice sheet model or by observations.  The englacial transfer term $\partial W_{en}/\partial t$ will be addressed below.
...
\subsection*{Discretization of the pressure evolution equation}  The pressure evolution equation \eqref{eq:pressureequation} is a nonlinear diffusion with additional ``reaction'' terms associated to opening and closing.  Unlike solving \eqref{eq:adeqn} for $W$, when solving \eqref{eq:pressureequation} for $P$ there is no dominating advection term.  Therefore we discretize it using an explicit centered, second-order scheme.  Again, because this is an explicit scheme, we consider stable time steps immediately.
...
\begin{equation}
\Delta t_P\, \left(\frac{2 \rho_i \max H k \max(W^\alpha)}{\rho_w E_0}\right) \left(\frac{1}{\Delta x^2} + \frac{1}{\Delta y^2}\right) = \frac{1}{2} \label{eq:dtDIFFP}
\end{equation}
...
\end{equation}
In fact, with the estimates $\rho_w/\rho_i \approx 1$, $E_0\approx 1$ m, and $\max H \approx 1000$ m we can show in the $\alpha=1$ case that we have $\Delta t_P$ which is about 1000 times smaller than $\Delta t_W$.  With these values and others used earlier (i.e.~$\Delta x = \Delta y = 500$ m, $\max |\bV|=10^5$ m/a, $k=10^{-2}$ m/s and $\max W=1$ m) we get
\begin{align*}
...
Now, the scheme we use for \eqref{eq:pressureequation} is similar to \eqref{eq:Wfd} for \eqref{eq:adeqn} but without a need for approximating advection:
\begin{align}
\frac{E_0}{(P_o)_{i,j}} \frac{P_{i,j}^{l+1} - \Plij}{\Delta t} &= \frac{1}{\rho_w g} \bigg[\frac{k W_e^\alpha \left(\psi_{i+1,j}^l - \psi_{i,j}^l\right) - k W_w^\alpha \left(\psi_{i,j}^l - \psi_{i-1,j}^l\right)}{\Delta x^2}  \label{eq:Pfd} \\
...
\begin{align}
P_{i,j}^{l+1} &= \Plij +  \frac{(P_o)_{i,j}}{E_0} \bigg[\omega_x k W_e^\alpha \left(\psi_{i+1,j}^l - \psi_{i,j}^l\right) - \omega_x k W_w^\alpha \left(\psi_{i,j}^l - \psi_{i-1,j}^l\right) \label{eq:Pfdupdate} \\
      &\qquad\qquad\qquad\quad + \omega_y k W_n^\alpha \left(\psi_{i,j+1}^l - \psi_{i,j}^l\right) - \omega_y k W_s^\alpha \left(\psi_{i,j}^l - \psi_{i,j-1}^l\right) \notag \\
      &\qquad\qquad\qquad\quad + \Delta t\, \left(\mathcal{C}_{ij} - \mathcal{O}_{ij} + \frac{m_{ij} + S_{ij}}{\rho_w}\right)\bigg]. \notag
\end{align}
...
\item FIXME: evaluate $\partial W_{en}/\partial t$
...
We use the default values in Table \ref{tab:constants} and \ref{tab:verifconstants}.  The exact solution is shown in Figures \ref{fig:Wexact} and \ref{fig:Pexact}.  We do a one month run on grids with spacing decreasing by factors of two from $5$ km to $156$ m.  Figure \ref{fig:refineWPpism} shows the results based on first-order upwinding for the fluxes.
...
\caption{Left: Average water thickness error $|W-W_{exact}|$ decays as $O(\Delta x^{0.77})$.  Right: Average pressure error $|P-P_{exact}|$ decays as $O(\Delta x^{1.09})$}
...
Explicit numerical scheme \eqref{eq:Wfd} for the mass conservation PDE \eqref{eq:adeqn}, combined with the first-order upwind case of formulas \eqref{eq:adfluxes}, is sufficiently simple so that we can analyze its properties.  For this scheme we now sketch a maximum principle argument which shows stability \citep{MortonMayers}.  The argument also shows positivity \citep{HundsdorferVerwer2010} as long as the total water input is nonnegative, although only the case $m_{ij}=S_{ij}=\partial W_{en}/\partial t = 0$ is given here.  We assume $\alpha=0$ for simplicity.  We consider only the case where all of the discrete velocities at the centers of cell interfaces are nonnegative: $\alpha_e\ge 0$, $\alpha_w\ge 0$, $\beta_n\ge 0$, $\beta_s\ge 0$.  The other upwinding cases, wherein these velocity components have various signs, can be handled by similar special-case arguments like the present one.
...
\begin{align*}
 W_{i,j}^{l+1} &= (\nu_x \alpha_w + \mu_x W_w) W_{i-1,j}^l + (\mu_x W_e) W_{i+1,j}^l + (\nu_y \beta_s + \mu_y W_s) W_{i,j-1}^l + (\mu_y W_n) W_{i,j+1}^l \\
      &\qquad + \Big[1 - \nu_x \alpha_e - \nu_y \beta_n - \mu_x (W_e + W_w) - \mu_y (W_n + W_s)\Big] \Wlij
\end{align*}
...
It follows from \eqref{eq:lincomb} that if $\Wlij\ge 0$ for all $i,j$ then $W_{ij}^{l+1}\ge 0$.

We see that all coefficients in linear combination \eqref{eq:lincomb} are nonnegative if the time step restrictions are satisfied.  Furthermore the coefficients add to one.  It follows that scheme \eqref{eq:Wfd} with first-order upwinding is positivity-preserving.  It also follows that the scheme is stable under conditions \eqref{eq:dtCFL} and \eqref{eq:dtDIFFW}, the conditions which describe all of the upwinding cases, thereby generalizing \eqref{eq:adstabcond} and \eqref{eq:diffstabcond}, respectively.
...
A distributed extension of the \cite{Bartholomausetal2011} theory is not, however, viable without a Darcy or other flux expression for $Q$.  This was not needed in the original theory because the lumped input and output from the hydrological system were available as data.
...
\end{equation}
This equation is just like equation \eqref{eq:PofWsteady} in the main text.  In steady state the pressure is a function of the amount of water.  It is interesting to also observe, however, that \eqref{eq:barth:steadypressure} shows that the water pressure depends not at all on the englacial macroporosity $\phi$.  Though the pressure is parameterized englacially by $P=\rho_w g z_w$ in \cite{Bartholomausetal2011}, its \emph{steady} value is entirely determined by the balance between sliding, wall melt, and creep closure in the subglacial system.  The englacial system is passive in determining the steady state pressure.

Returning to equation \eqref{eq:barth:distpressure} we can now connect the theory outlined in this Appendix with the main theory in the paper.  To start, note that under any reasonable Darcy-type formulation for the flux $Q$ in \eqref{eq:barth:distpressure}, the $\phi\to 0$ limit of \eqref{eq:barth:distpressure} is an elliptic equation for the water pressure.  This limit is an equation which is similar to the elliptic ($\phi\to 0$) limit of \eqref{eq:pressureequation} in the main text.  Thus the $\phi\to 0$ limit of the subglacial-plus-englacial theory of \cite{Bartholomausetal2011} already appears in \cite{Schoofetal2012}.  The main text simulaneously describes the distributed extension of the \cite{Bartholomausetal2011} theory and the englacial storage extension of the \cite{Schoofetal2012} theory.
\end{document}
