...
At this point we have an essentially-complete new mathematical model, consisting of conservation equation \eqref{eq:conservewithPen} for evolution of $W$, equation \eqref{eq:pressureequation} for evolution of pressure $P$, and the bounds on $W$ and $P$.  The conductivity function $K(W)$ remains to be specified.

FIXME: text and image of $K(W)$ removed; plan to use \eqref{eq:fluxalt}
...
This form of the mathematical model may surprise some readers, but we have written it in a way that allows three particular limits to be clear:
\begin{itemize}
\item The $\phi\to 0$ limit is the model described by \cite{Schoofetal2012}.
\item The $P \to P_o$ and $W\ll H$ (and $\phi \to 0$) limit is a standard model for locating subglacial lakes \cite{Siegertetal2009} in which the pressure is no longer unknown and thus any model of cavity evolution is inactive:
\begin{empheq}[box=\mybluebox]{align}
W &\ge 0, \notag \\
\bV &= - \frac{K(W)}{\rho_w g} \grad\left(P_o + \rho_w g b\right), \phantom{ldsfj} \label{eq:lakesmodel} \\
\phantom{ldsfj} \frac{\partial W}{\partial t} + \Div\left(\bV\, W\right) &= \left[\frac{m+S}{\rho_w}\right]. \notag
\end{empheq}
\end{itemize}
