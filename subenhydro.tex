...
Any reasonable dynamical model of the liquid water underneath and within a glacier or ice sheet has at least these two elements: the mass of the water is conserved and the water flows from high to low hydraulic potential \citep{Clarke05}.  Then there are many variations considered in the existing literature.  Physical processes also control the geometry of the linked cavities \citep{Kamb1987} or conduits (channels) \citep{Nye1976} in which the water moves, including the opening of cavities by sliding of the overlying ice past bedrock bumps (i.e.~cavitation), melt on the walls of cavities and conduits, and the closure of cavities and conduits by creep \citep{Hewitt2011}.  Water could be exchanged with a porous englacial system \citep{Bartholomausetal2011}.  The evolving pressure and amount of water in conduits could evolve by physical processes, but the existing theory of conduits apparently requires their locations to be fixed a priori \cite{PimentelFlowers2011,Schoofmeltsupply}.

This paper describes a model for distributed systems of linked subglacial cavities, with connection to additional englacial storage.  The cavities open by sliding of the ice and wall melt and they close by ice creep.  These physical processes combine to determine the local relationship between water amount and water pressure \citep{Schoofetal2012}.  Channels are not, however, included in the model.
%FIXME:?  Wall melt is included as a contribution to the mass conservation equation but not in the cavity evolution equation.

In our model pressure is determined non-locally over each connected component of the hydrological system, and no prior functional relation between subglacial water amount and pressure is assumed \citep[compare]{FlowersClarke2002_theory}.  The connection to a macroporous englacial system implies that the subglacial water pressure solves an equation which is a parabolic approximation of the distributed pressure equation given in elliptic variational inequality form by \cite{Schoofetal2012}.  Unlike the \cite{Schoofetal2012} model in which an elliptic variational inequality for pressure must be solved at each time step, at the cost of some computational inefficiency we can avoid solving an instantaneous distributed balance by not exactly enforcing the full-cavity condition.  Avoiding the elliptic solve eases implementation and parallelization.

After deriving a fundamental advection-diffusion form of the model from more fundamental physical principles we consider cavity evolution and some alternatives for closures which can determine the pressure.  Then we consider the equations which apply in steady state and we show that pressure is function of subglacial water amount in that case.  In the steady radial case we compute an exact solution for subglacial water amount and pressure, a useful tool for verification.  Then we present a (parallel) numerical scheme with particular attention to time step restrictions and the treatment of advection.  The first presented numerical results are for the verification case, wherein we can show convergence under grid refinement.  After that we show results for the Nordenskioldbreen tidewater glacier in Svalbard.


\section{Elements of a sub- and en-glacial hydrology continuum model}

\subsection*{Mass conservation}  We assume that liquid water in a glacier is incompressible and of constant density.  Thus the thickness of a water layer tells us its mass, and thus our fundamental statement of mass conservation (below) will describe the evolution of water thickness.  Choosing to model subglacial hydrology using a water thickness as the state variable is not a significant restriction on the physics because specific subglacial physics comes from choosing a form for the water flux and a closure for the pressure.

Denote the thickness of the subglacial water layer by $W(t,x,y)$.  This thickness is only meaningful compared to observations if it is regarded as an average over a horizontal scale of tens to thousands of meters \citep{FlowersClarke2002_theory}.  Though the hydrologic system has fine spatial variation which one is unable to model with cells of kilometer size, we  attempt to model spatially-averaged versions of water amount and water pressure.

Additionally we allow water to be stored englacially in a system of crevasses, cracks, veins, and/or moulins of unspecified morphology but with overall macro porosity $\phi$ \citep{Bartholomausetal2011}.  (Throughout this paper the case $\phi=0$ will be considered.  In this case there is no englacial storage.  This case may apply to cold glaciers and the interior of ice sheets.  The extension that $\phi$ varies in time and map-plane location ($\phi(t,x,y)$) can also be considered.)
% FIXME:  maybe we should allow phi(x,y) right away
The mass of this water can be tracked with an elevation above the bed $z_{en}(t,x,y)$ for the top of the (notional) ``water table'' in the porous glacier.  In a column of the glacier with cross-sectional area $\Delta x \times \Delta y = \Delta a$ the volume of water is then $\phi\, \Delta a \, z_{en}$ \citep{Bartholomausetal2011}.  But then we can just as easily track the water by an effective thickness for the englacial water, say
\begin{equation}
W_{en}(t,x,y) = \frac{\phi\, \Delta a\, z_{en}}{\Delta a} = \phi\, z_{en}. \label{eq:definezen}
\end{equation}
Thus the total effective thickness of the water in a glacier at map-plane location $(x,y)$ and time $t$ is $W(t,x,y)+W_{en}(t,x,y)$.

Suppose $\bq$ is the vector water flux (units $\text{m}^2\,\text{s}^{-1}$) and $\Phi$ is a source term ($\text{m}\,\text{s}^{-1}$).  In two spatial dimensions the mass conservation equation is \citep{Clarke05}
\begin{equation} \label{eq:conserve}
\frac{\partial W}{\partial t} + \frac{\partial W_{en}}{\partial t} + \Div \bq = \Phi
\end{equation}
...
We separate the water sources between the melt on the lower surface of the glacier versus supraglacial drainage.  Let $\rho_w$ be the density of fresh liquid water, $m$ be the rate at which basal melting (refreeze) of ice adds (removes) water, and $S$ be the rate at which surface runoff adds water to the subglacier layer.  Then the source term in equation \eqref{eq:conserve} has this form:
\begin{equation}
\Phi = \rho_w^{-1} \left(m + S\right).  \label{eq:drainagesplit}
\end{equation}


\subsection*{Hydraulic potential and Darcy flow}  The water flux $\bq$ in equation \eqref{eq:conserve} is related to the gradient of a hydraulic potential (head) $\psi(t,x,y)$ for the subglacial water.  This quantity combines the actual subglacial water pressure $P(t,x,y)$ and the gravitational potential of the top of the water layer,
...
\subsection*{Closures without cavity geometry evolution}  To start we briefly consider three simple closures which appear in the literature but which do not use equation \eqref{eq:hewittcapacity} or similar.  Their consequences emerge as limiting cases of our more complete theory, but only near steady state.  These closures are distinguished both by their physical motivation and by the mathematical form they imply for the mass conservation equation.

The first is the assumption that the pressure is equal to the overburden pressure,
\begin{equation}
P = P_o.\label{eq:Pisoverburden}
\end{equation}
This model is sometimes used for routing subglacial water under ice sheets so as to identify subglacial lake locations \citep[for example]{Siegertetal2009}.  Straightforward calculations using the simplification $W_{en}=0$ which applies to cold ice sheets, equations \eqref{eq:conserve} and \eqref{eq:flux} with $K(W)=K_0$, and equation \eqref{eq:Pisoverburden} show that the more general mass conservation equation \eqref{eq:adeqn} becomes an advection with an ice-geometry-determined velocity.  Specifically,
\begin{equation}
...
\end{equation}
In fact the first diffusion term ``$\Div\left(K_0 \,W\, \grad W\right)$'' on the right of \eqref{eq:PisoverConservation} is usually not stated because the approximation $W\ll H$ is accepted, so that the hydraulic potential is insensitive to the water layer thickness, i.e.~$\psi = P_o + \rho_w g b$.  Under this common simplification, this model becomes a pure advection with a velocity $\tilde\bV$ which is independent of $W$.  In that case equation \eqref{eq:PisoverConservation} possesses characteristic curves \citep{Evans} which are the trajectories of the water flow.  The more complete models we consider in this paper do not have such characteristic curves because they have a velocity dependent directly and indirectly on the gradient of the water amount $W$ which is being transported.

Regarding modeling with the form stated here, i.e.~equation \eqref{eq:PisoverConservation} \emph{with} the diffusion term, the continuum model is actually well-posed.  The continuum solution has finite water layer thickness at all times.  By contrast the simplified model without the diffusion term will exhibit continuum solutions with infinite concentration at every location where the simplified potential $\psi = P_o + \rho_w g b$ has a minimum.  Such a model is not well-posed and its numerical implementation is badly-behaved under grid refinement.

At an almost opposite extreme in terms of the mathematical form, as a second closure one might hypothesize that the water pressure is determined by the amount of water.  \cite{FlowersClarke2002_theory} propose the closure
\begin{equation}
